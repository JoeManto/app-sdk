...
final class IAPManager: NSObject {
    ...
    func fetchProducts(products: [Product]) {
        let request = SKProductsRequest(
            productIdentifiers: Set(products.map { $0.id })
        )
    ...
    func purchase(_ product: Product) {
        ...
        guard let storeKitProduct = products.first(where: { $0.productIdentifier == product.id }) else {
    ...
    func paymentQueueRestoreCompletedTransactionsFinished(_ queue: SKPaymentQueue) {
        if queue.transactions.count == 0 {
            AppSDKUtil.showStoreKitAlert(
                title: NSLocalizedString("No Purchases Restored", comment: ""),
                msg: NSLocalizedString("This Apple ID hasn't previously made a purchase", comment: "")
            )
        }
        else {
            let title = NSLocalizedString("%@ Purchases Restored", comment: "").replacingOccurrences(of: "%@", with: "\(queue.transactions.count)")
            AppSDKUtil.showStoreKitAlert(title: title, msg: "")
        }
